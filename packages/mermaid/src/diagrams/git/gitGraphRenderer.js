...
const findClosestParent = (parents) => {
  ...
  parents.forEach((parent) => {
    const parentPosition = dir === 'TB' ? commitPos.get(parent).y : commitPos.get(parent).x;
  ...
  sortedKeys.forEach((key) => {
    ...
    if (isParallelCommits) {
      if (commit.parents.length) {
        ...
        pos =
          dir === 'TB'
            ? commitPos.get(closestParent).y + commitStep
            : commitPos.get(closestParent).x + commitStep;
      } else {
        pos = 0;
        if (dir === 'TB') {
          pos = 30;
    ...
    const y = dir === 'TB' ? posWithOffset : branchPos.get(commit.branch).pos;
    const x = dir === 'TB' ? branchPos.get(commit.branch).pos : posWithOffset;
