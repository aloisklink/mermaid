...
import { defaultConfig, type MermaidConfigOptions } from '../config.js';
...
describe('diagram-orchestration', () => {
  ...
  describe('proper diagram types should be detetced', () => {
    ...
    it('should detect proper flowchart type based on config', () => {
      ...
      // graph & dagre-d3 => flowchart
      expect(
        detectType('graph TD; A-->B', config({ flowchart: { defaultRenderer: 'dagre-d3' } }))
      ).toBe('flowchart');
      // flowchart & dagre-d3 => error
      expect(() =>
        detectType('flowchart TD; A-->B', config({ flowchart: { defaultRenderer: 'dagre-d3' } }))
      ...
      expect(
        detectType('graph TD; A-->B', config({ flowchart: { defaultRenderer: 'dagre-wrapper' } }))
      ).toBe('flowchart-v2');
      ...
      expect(
        detectType('flowchart TD; A-->B', config({ flowchart: { defaultRenderer: 'elk' } }))
      ).toBe('flowchart-elk');
    });
